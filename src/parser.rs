...
pub fn parse(input: &str) -> Query {
    ...
    fn length_range(
        ...
        match p {
            ...
            Conjunction(_) | Disjunction(_) => todo!(),
   ...
   pub  struct ExecutionContextBuilder<'a> {
    ...
    impl<'a, 'b, 'c> ExecutionContext<'a, 'b> {
        fn nested_constraints_execute(&mut self, candidate: &'a str, pattern: &[Constraint]) {
            if pattern.len() == 0 && self.patterns.len() == self.candidate_stack.len() {
                if candidate.len() == 0 {
            ...
            'streaks: for streak_len in streak_length_bound.0..=streak_length_bound.1 {
                ...
                match &pattern[constraint_index] {
                    ...
                    Variable(v) => {
                        // println!("Submatchv {:?}; {:?} p={:?},{}, {:?} for candidate {:?} [{}-{}]", &streak_length_bound, &anchored_left, pattern, v, bindings[*v], candidate_stack, streak_start, streak_end);
                        ...
                        for var_distinct_constraint in &self.spec_var_inequality {
                            ...
                            {
                                if self.bindings[a].is_some()
                                    && self.bindings[a] == self.bindings[b]
                                {
...
pub mod tests {
    ...
    fn executort() {
        ...
        let mut cb = |a: &Vec<&str>, b: &VariableMap<Option<&str>>| {
            println!( "Bound sol'n {:?} @{:?}", a, b.0.iter().cloned().filter_map(|v| v).collect_vec());
